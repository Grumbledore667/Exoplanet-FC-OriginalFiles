...
function CTradingUI:onTradeAccept()
   ...
   if buySum == sellSum and buySum ~= 0 then
      for _,itemTable in ipairs(self.buyInventory.items) do
         trader:getTradeInventory():giveItemTo(itemTable.item, player:getInventory(), itemTable.count)
      end
      for _,itemTable in ipairs(self.sellInventory.items) do
         player:getInventory():giveItemTo(itemTable.item, player.exchangeTarget:getTradeInventory(), itemTable.count, true)
      end
