...
function console:init()
   ...
   self.cmdBox = self.wnd:getChild("Command")
...
local function cbConsoleCommandKeyDown(args)
   ...
   local cmd = CEGUI.toEditbox(console.cmdBox)
   ...
   elseif keyArgs.scancode == 72 then -- UP
      ...
      return true
   elseif keyArgs.scancode == 80 then -- DOWN
      if not historyScroll then -- CTRL
         scrollAutocomplete(1)
      else
         scrollHistory(-1)
...
declareConsoleCommand {
   ...
   func = function(self)
      ...
      taid_running = debugwidget(function()
         local char = raycastCharacterOnScreen(vec3New{x=0.5, y=0.5}, false)
...
declareConsoleCommand{
   ...
   func = function(self)
      if isMainMenu() or not isPlayerPresent() or not self.testName then return end
      local path
