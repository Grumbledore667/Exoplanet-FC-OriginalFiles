...
function CInventoryPlayerUI:init()
   ...
   GUIUtils.widgetSubscribeEventProtected(self.wnd:getChild("CraftButton"), "Clicked", partial(self.onCraftClick, self))
...
function CInventoryPlayerUI:show(state)
   CInventoryBaseUI.show(self, state)
end
...
--CALLBACKS
function CInventoryPlayerUI:onCraftClick(args)
   gameplayUI.annoyingHintUI:completeTask("cactus")
   gameplayUI.craftUI:show(not gameplayUI.craftUI:isVisible())
end
