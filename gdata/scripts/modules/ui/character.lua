...
function CCharacterUI:setStatisticsCombat(item)
   if item then
...
function CCharacterUI:onCharacterSlotClick(args)
   ...
   if click_args.button == CEGUI.RightButton then
      ...
      if item then
         gameplayUI.popupMenuUI.item = item
         gameplayUI.popupMenuUI:setup({"unequip"})
...
function CCharacterUI:onCharacterSlotMouseMove(args)
   ...
   local item = getMC():getInventory():getSlotItem(slotID or slotName)
...
function CCharacterUI:onDragAndDropCharacterSlot(args)
   ...
   if sourceInventoryName == "inventoryContainer" or sourceInventoryName == "inventoryTurret" then
      ...
      item = pInventory:getItemById(itemID)
   end
   ...
   elseif item:isEquipable() and (item:getItemName() ~= "hand_to_hand.wpn" or targetSlotName == "slot1") then
      ...
      elseif item:getFastAccessSlot() == slotID then
         player:unequipItem(item)

      elseif item == player:getWeaponSlotItem() and item:getFastAccessSlot() ~= slotID then
         pInventory:setItemToFastAccess(slotID, item:getId())
         self:setup()

      elseif type(slotID) == "number" then
         if pInventory:setItemToFastAccess(slotID, item:getId()) then
            pInventory:equipSlotWithItem(slotID, item:getId(), false, false)
         end
      end
   end
end
