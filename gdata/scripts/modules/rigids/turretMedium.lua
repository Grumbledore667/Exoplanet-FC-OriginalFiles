...
function CTurretMedium:OnCreate()
   ...
   self.sounds = {}
   self.sounds.move = self:createAspect( "turret_move.wav" )
...
function CTurretMedium:OnInteractTriggerBegin( obj )
   if obj == getDefaultPlayer() and not objInDist(obj:getPose():getPos(), self:getPose():getPos(), self.scanDist) then
      return
   end
