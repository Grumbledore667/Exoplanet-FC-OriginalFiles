...
function CShroomBomb:OnCreate()
   ...
   self.sounds.idle = self:createAspect( "shroom_idle.wav" )
   ...
   self.sounds.death = self:createAspect( "shroom_explosion.wav" )
...
function CShroomBomb:dps()
   ...
   for _, char in pairs(self.chars) do
      if objInDist(self:getPose():getPos(), char:getPose():getPos(), self.burstRunDistance) then
         local t = {health = -self.cloudDPS*1.1, duration = 1.1, name = "shroom_poison", refresh = true, sounds = {"cough"}}
...
function CShroomBomb:OnLoadState(state)
   if state.startled and state.burstDelay > 0 then
