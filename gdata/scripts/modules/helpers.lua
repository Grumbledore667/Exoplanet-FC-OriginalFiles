...
function helpers.insertSoundToTable(obj, filename, t, options)
   ...
   if options.fadeIn then
      snd:setFadeIn(options.fadeIn)
   end
   if options.fadeIn then
      snd:setFadeOut(options.fadeIn)
   end
...
---@param humanMaleCharacter CCharacter
function helpers.createJointSmokingEmitterAspect(humanMaleCharacter)
   local smokeAspect = humanMaleCharacter:createAspect("koster_smoke.sps")
   smokeAspect:setLoop(true)
   smokeAspect:getPose():setParent(humanMaleCharacter:getBonePose("head_slot"))
   smokeAspect:getPose():resetLocalPose()
   smokeAspect:getPose():setLocalPos(vec3New{z=-13, y=-13})
