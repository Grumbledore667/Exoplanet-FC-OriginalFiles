...
function CDrone:OnCreate()
   ...
   runTimer( 0, self, function (self)
      self.repairTarget = loadParamObjects( self, "repairTarget", nil )

      if ( self.repairTarget and #self.repairTarget > 0 ) then
         self.repairTarget = self.repairTarget[1]
      else
...
function CDrone:OnIdle()
   if ( self.canRepair and not self:getState( "patrol" ) ) then
      ...
      if ( not self.fx.omniTimer ) then
         self.fx.omniTimer = runTimer( 0.08, self, function (self) self.fx.omni:setIntensity( 0.5 + rand( 2 ) ) end, true )
      end
...
function CDrone:idle_start()
   ...
   if ( self.repairTarget ) then
      ...
      if ( self.canRepair ) then
         self.animationsManager:loopAnimation( "repair.caf" )
