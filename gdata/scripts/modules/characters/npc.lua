...
function CNPC:initFX()
   ...
   if not self.smokeFX and prefabName == "human_male.cfg"  then
...
function CNPC:consumeItem_running()
   ...
   local spawnItemEvent
   local action
   if item:isFood() then
      action = "eat"
      slotName = "item_slot2" --current eating animations are left-handed
      spawnItemEvent = "spawn_item"
   elseif item:isDrink() then
      action = "drink"
      spawnItemEvent = "drink_start"
   ...
   if animation and animation ~= "" then
      ...
      if spawnItemEvent then
         coro.waitAnimationEventIn(self, animation, spawnItemEvent)
      end
   ...
   for _, effect in ipairs(item:getConsumeEffects()) do
...
function CNPC:_getAnimationFor(event, direction)
   ...
   elseif event == "eat" then
      return self.animations.idle.eat.left_handed
