...
function CShark:idle_start()
   self.animationsManager:loopAnimation( "dive.caf" )
   ...
   if ( not self.senseScheduler:getSense("inZone") ) then
      ...
      runTimer( 3+rand(3), self, self.move_run, false )
   end
end
...
function CShark:move_start()
   ...
   self.animationsManager:loopAnimation( "swim_fwd.caf" )
   ...
   if ( not self.senseScheduler:getSense("inZone") ) then
      ...
      self:setOrientationFull( rand(360) )
      runTimer( 3+rand(3), self, self.idle_run, false )
   end
end
...
function CShark:chase_start()
   self.animationsManager:loopAnimation( "fast_fwd.caf" )
...
function CShark:attack_start()
   ...
   self.animationsManager:loopAnimation        ( "dive.caf" )
   self.animationsManager:playAnimation        ( "jump.caf", false )
   self.animationsManager:subscribeAnimationEnd( "jump.caf", self, self.attackAnimStop )
   --self.animationsManager:subscribeAnimationEventOut( "jump.caf", "attack", self, self.attackAnimStop )
...
function CShark:attackAnimStop()
   ...
   runTimer( rand(2), self, self.coolDown, false )
end
...
function CShark:damage_start()
   local animation = "swim_left.caf"

   if ( rand(2) > 1 ) then
      animation = "swim_right.caf"
   end
