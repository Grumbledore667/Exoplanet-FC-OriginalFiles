...
\
      coro.waitAnimationEnd(player, \"getup_start\")\
\
      --Jack stretches muscles\
      player.animationsManager:playAction(\"getup_end\")\
\
      coro.waitAnimationEnd(player, \"getup_end\")\
\
...
\
      coro.waitAnimationEventIn(player, \"device_hand\", \"button\")\
\
      --Jack uses a scan\
      local scanner = player:getInventory():getItemByName(\"scanner.itm\")\
      player:runScanWave(scanner, false)\
\
      coro.waitAnimationEnd(player, \"device_hand\")\
\
      --Scene is done\
