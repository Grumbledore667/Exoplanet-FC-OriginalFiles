...
function helpers.assureMockRng()
   if not _G.CEGUI then
      package.loaded.rng = helpers.mockCallable
   end
end
...
--- dummy mock object with recursive indexing and calling metamethods that doesn't do anything
local mockCallable
mockCallable = setmetatable({}, {
   __call = function()
      return mockCallable
   end,
...
helpers.mockCallable = mockCallable
