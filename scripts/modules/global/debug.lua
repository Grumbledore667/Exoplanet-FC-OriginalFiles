...
deb.versionStr = "Alersteam ( Early Access Alpha build 0.36.1 )"
deb.versionInt = 13
...
function deb.dumpTableToFile(t, path, requirable, comment)
   ...
   if requirable then
      data = string.format("%sreturn %s", comment, pretty.writeSorted(t, nil, false))
   else
      data = comment
      for key, chunk in tablex.sort(t) do
         data = string.format("%s%s = %s \n", data, key, pretty.writeSorted(chunk, nil, false))
      end
   end
...
local function process(item)
   if type(item) == 'userdata' then
      ...
      if isCEGUIType(toluaType) then
         ...
         if mt then
            local engineTypeStr = registry[mt]
            if engineTypeStr then
               return string.format("<%s>", string.match(engineTypeStr, "^class ([%w:]+)") or "userdata")
            end
         end
      end
   end
