...
function CCrucasWeak:getLabel()
   return self.parameters.customLabel or "Juvenile Crucass"
end
...
function CCrucas:initFX()
   if not self.fx then
      ...
      self.fx.eatDeadBlood = self:createAspect("abori_damage.sps")
      self.fx.eatDeadBlood:setLoop(false)
      self.fx.eatDeadBlood:getPose():setParent(self:getBonePose("item_slot1"))
      self.fx.eatDeadBlood:getPose():resetLocalPos()
      self.fx.eatDeadBlood:disable()
   end
end
...
function CCrucas:loadDynamicParameters(params)
   ...
   self.parameters.customLabel = params.customLabel
end
...
function CCrucas:eat_dead_start()
   ...
   self.animationsManager:subscribeAnimationEventIn("eat", "blood1",function() self.fx.eatDeadBlood:play()end)
   self.animationsManager:subscribeAnimationEventIn("eat", "blood2",function() self.fx.eatDeadBlood:play()end)
   self.animationsManager:subscribeAnimationEventIn("eat", "blood3",function() self.fx.eatDeadBlood:play()end)

   local healing = self:getStatCount("healthMax")
   local t = { changeStats = { health = healing }, name = "crucas_eating", duration = 120, refresh = true, removeOnDeath = true}
   self.statusEffectsManager:addStatusEffect(t)
end

function CCrucas:eat_dead_stop()
   self.statusEffectsManager:destroyEffectsByName("crucas_eating")
end
...
function CCrucas:getLabel()
   return self.parameters.customLabel or "Crucass"
end
