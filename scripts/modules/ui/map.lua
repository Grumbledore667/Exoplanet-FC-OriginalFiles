...
function CMapUI:show(state)
   if state then
      if hasPlayerItem(self.currentMapItemName) then
         return CBaseModule.show(self, state)
      else
         gameplayUI:showInfoTextEx("You don't have a map of this area", "minor", "")
         return false
      end
   else
      return CBaseModule.show(self, state)
...
function CMapUI:onHide()
   ...
   gameplayUI.gameplayMenuUI:onHideTab("Map")
