...
function cbConfirmationKey(args)
   ...
   if keyArgs.scancode == getButtonCode("ESCAPE") then
      cbConfirmationNo()
   elseif keyArgs.scancode == getButtonCode("ENTER") then
      cbConfirmationYes()
   end
end
...
local function countWidgets()
   local count = 0
   for k,v in pairs(debugWidgets) do
      count = count + 1
   end
   return count
end

local debugWidgetID = 0
...
function debugwidget(func, updateRate, ...)
   if not isMainMenu() and pcall(func, ...) then
      ...
      debugWidgetID = debugWidgetID + 1
      wnd:setName("DebugWidget" .. debugWidgetID)
      gameplayUI.wndGameplay:addChild(wnd)

      local yoffset = (wnd:getHeight().offset + 3) * (countWidgets() - 1)
