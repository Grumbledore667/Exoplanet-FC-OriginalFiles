...
function options:init()
   ...
   --Sound
   self.restoreAudioDefaultsButton = self.wnd:getChild("System/Audio/RestoreDefaultsButton")
   GUIUtils.widgetSubscribeEventProtected(self.restoreAudioDefaultsButton, "Clicked", cbRestoreAudioDefaults)
   self.soundBars = {
      master = CEGUI.toScrollbar(self.wnd:getChild("System/Audio/MasterVolume/Scrollbar")),
      sfx = CEGUI.toScrollbar(self.wnd:getChild("System/Audio/SfxVolume/Scrollbar")),
      voice = CEGUI.toScrollbar(self.wnd:getChild("System/Audio/VoiceVolume/Scrollbar")),
      music = CEGUI.toScrollbar(self.wnd:getChild("System/Audio/MusicVolume/Scrollbar")),
   }
   GUIUtils.widgetSubscribeEventProtected(self.soundBars.master, "ScrollPositionChanged", cbMasterVolumeChanged)
   GUIUtils.widgetSubscribeEventProtected(self.soundBars.sfx, "ScrollPositionChanged", cbSfxVolumeChanged)
   GUIUtils.widgetSubscribeEventProtected(self.soundBars.voice, "ScrollPositionChanged", cbVoiceVolumeChanged)
   GUIUtils.widgetSubscribeEventProtected(self.soundBars.music, "ScrollPositionChanged", cbMusicVolumeChanged)
   --
   ...
   if isEditor() then
      self.resolutionCombobox:setProperty("Disabled", "true")
   end
...
function options:updateOptions()
   self.soundBars.master:setProperty("ScrollPosition", getMasterVolume())
   self.soundBars.sfx:setProperty("ScrollPosition", getSfxVolume())
   self.soundBars.voice:setProperty("ScrollPosition", getVoiceVolume())
   self.soundBars.music:setProperty("ScrollPosition", getMusicVolume())
...
function cbRestoreAudioDefaults(args)
   resetSoundsVolume()
   options:updateOptions()
   options.saveButton:setProperty("Disabled", "false")
end

function cbMasterVolumeChanged(args)
   local click_args = CEGUI.toMouseEventArgs(args)
   local value = click_args.window:getScrollPosition()
   setMasterVolume(value)
   options.saveButton:setProperty("Disabled", "false")
end

function cbSfxVolumeChanged(args)
   local click_args = CEGUI.toMouseEventArgs(args)
   local value = click_args.window:getScrollPosition()
   setSfxVolume(value)
   options.saveButton:setProperty("Disabled", "false")
...
function cbVoiceVolumeChanged(args)
   local click_args = CEGUI.toMouseEventArgs(args)
   local value = click_args.window:getScrollPosition()
...
function cbKeyDown(args)
   ...
   if options.keyAssignOverlay:isVisible() and options.keyButtonInFocus then
      if keyArgs.scancode == getButtonCode("ESCAPE") then
         ...
         Keys.setButtonCode(buttonName, keyArgs.scancode)
         options:updateOptions()
         Keys.saveKeysMap()
      end
   else
...
local function convertCEGUIMouseCode(code)
   local map = {
      [0] = 238,
      [1] = 240,
      [2] = 239,
...
function cbMouseButtonDown(args)
   local click_args = CEGUI.toMouseEventArgs(args)
   local mouseCode = convertCEGUIMouseCode(click_args.button)
