...
local INTERACT_LABEL_TIME = 4
local DWELL_TIME = 1.5
...
function CHudUI:init()
   ...
   self.focus = {currentObj = nil, label = nil, updateTimer = nil, focusStartTime = nil}
...
function CHudUI:getFocusObj()
   return self.focus.currentObj
end

function CHudUI:setFocusObj(obj)
   local curObj = self.focus.currentObj
   if curObj == obj then
      if curObj and self.focus.focusStartTime and not self.focus.dwellFired then
         if getGameTime() - self.focus.focusStartTime >= DWELL_TIME then
            questSystem:fireEvent("dwell", curObj:getName(), curObj)
            self.focus.dwellFired = true
         end
      end
      return
   end
   ...
   if curObj then
      ...
      if curObj.unsubscribeOnDestroy then
         curObj:unsubscribeOnDestroy(self.focus.onDestroyCallback)
         self.focus.onDestroyCallback = nil
      end
   end

   self.focus.currentObj = obj
   self.focus.interactLabel:setVisible(false)
   self.focus.focusStartTime = getGameTime()
   self.focus.dwellFired = false
   ...
   if obj then
      ...
      if obj.subscribeOnDestroy then
         self.focus.onDestroyCallback = obj:subscribeOnDestroy(self.setFocusObj, self)
      else
...
function CHudUI:updateFocusUI()
   ...
   if obj and not getGlobalParam("inScene") then
      ...
      if getGameTime() - self.focus.focusStartTime >= INTERACT_LABEL_TIME and obj.getInteractLabel and not obj.OnDie then
