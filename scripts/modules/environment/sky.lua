...
function SkySystem:updateSky()
   ...
   for _,callback in ipairs(tablex.copy(self.subscribersDayTime)) do
...
--Isn't protected from duplicates.
...
function SkySystem:updateDayState(dayState)
   ...
   if self.previousDayState ~= self.dayState or (self.preFFDay and self.preFFDay ~= self:getCurrentDay()) then
      ...
      for _,callback in ipairs(tablex.copy(self.subscribersDayStateChange)) do
...
function SkySystem:fastForwardTime(timeUnit)
   ...
   --copy so the subbers can unsub and won't break the order
   for _,callback in ipairs(tablex.copy(self.subscribersFastForwardTime)) do
      callback.func(event, unpack(callback.args))
   end
   for _,callback in ipairs(tablex.copy(self.subscribersFastForwardTimeEnd)) do
      callback.func(event, unpack(callback.args))
   end
   self.preFFDay = nil
